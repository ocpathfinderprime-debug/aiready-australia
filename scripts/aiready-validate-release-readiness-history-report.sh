...
usage() {
  cat <<'EOF'
Usage:
  aiready-validate-release-readiness-history-report.sh /path/to/history-audit-root

Validate that an AIReady release-readiness history audit root contains the
...
required_readme_strings=(
  "# AIReady Release Readiness History Audit"
...
required_artifact_audit_strings=(
  "AIReady release-readiness history artifact audit"
  "audit_root: $audit_root"
  "$summary_readme"
  "$summary_manifest"
  "$validation_log"
...
for report_root in "${report_roots[@]}"; do
  if ! [[ "$report_root" =~ /reports/aiready-release-readiness-[0-9-]+-AWST$ ]]; then
    echo "Invalid report_root path shape: $report_root" >&2
    exit 1
  fi
  require_nonempty_file "$report_root/release-readiness-report-validation.txt"
  require_nonempty_file "$report_root/proof-sha256-validation.txt"
  require_literal "$report_root" "$artifact_audit"
  require_literal "$report_root/release-readiness-report-validation.txt" "$artifact_audit"
...
report_packet_count="$(grep -Ec '^  - .*/reports/aiready-release-readiness-[0-9-]+-AWST/(release-readiness-report-validation\.txt|proof-sha256-validation\.txt)$' "$artifact_audit" 2>/dev/null || printf '0\n')"
...
required_checksum_strings=(
  "# AIReady Release Readiness History Proof Checksums"
  "audit_root: $audit_root"
  "  $summary_readme"
  "  $summary_manifest"
  "  $validation_log"
...
required_checksum_validation_strings=(
  "AIReady release-readiness history proof checksum validation"
  "audit_root: $audit_root"
  "$summary_readme: OK"
  "$summary_manifest: OK"
  "$validation_log: OK"
