...
usage() {
  cat <<'EOF'
Usage:
  aiready-normalize-all-release-readiness-history-proofs.sh [reports_root]

Find stale AIReady release-readiness history audit packets that predate the
...
normalizer="$workspace_root/scripts/aiready-normalize-release-readiness-history-proof.sh"
...
mapfile -t audit_roots < <(find "$reports_root" -maxdepth 1 -type d -name 'aiready-release-readiness-history-[0-9]*-AWST' | sort)

if [[ "${#audit_roots[@]}" -eq 0 ]]; then
  echo "No release-readiness history audit roots found under $reports_root"
  exit 0
fi
...
if [[ "${#stale_roots[@]}" -eq 0 ]]; then
  echo "No stale release-readiness history audit roots found"
...
printf 'Normalizing %s stale release-readiness history audit root(s)\n' "${#stale_roots[@]}"
printf '%s\n' "${stale_roots[@]}"
...
echo
printf 'Normalized stale release-readiness history audit root(s): %s\n' "${#stale_roots[@]}"
if [[ "${#skipped_roots[@]}" -gt 0 ]]; then
  echo "Skipped structurally partial roots:"
  printf '%s\n' "${skipped_roots[@]}"
fi
