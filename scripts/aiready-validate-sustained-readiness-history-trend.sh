...
usage() {
  cat <<'EOF'
Usage:
  aiready-validate-sustained-readiness-history-trend.sh [output_root] [count]

Validate the latest N AIReady sustained-readiness history audit roots and
capture one aggregate trend packet proving the history-audit lane remains
consistent.

Defaults:
  output_root -> reports/aiready-sustained-readiness-history-trend-<timestamp>
  count       -> 5
EOF
}
...
output_root="${1:-$workspace_root/reports/aiready-sustained-readiness-history-trend-$(date +%Y-%m-%d-%H%M-AWST)}"
count="${2:-5}"
validator="$workspace_root/scripts/aiready-validate-sustained-readiness-history-report.sh"
...
mapfile -t audit_roots < <(
  ls -1dt "$workspace_root"/reports/aiready-sustained-readiness-history-20*-AWST 2>/dev/null |
    head -n "$count"
)

if [[ "${#audit_roots[@]}" -eq 0 ]]; then
  echo "No sustained-readiness history audit roots found under $workspace_root/reports" >&2
  exit 1
fi
...

{
  ...
  for i in "${!status_lines[@]}"; do
    ...
    if [[ "$checked" != "$count" ]]; then
      comparable_flag="requested-window-mismatch"
    fi
...

{
  ...
  for i in "${!status_lines[@]}"; do
    ...
    printf '    {"status":"%s","audit_root":"%s","reports_checked":%s,"reports_passed":%s,"reports_failed":%s,"mode":"%s"}%s\n' \
      "$status" "$audit_root" "${checked_counts[$i]}" "${pass_counts[$i]}" "${fail_counts[$i]}" "$mode" "$suffix"
  done
  printf '  ]\n'
  printf '}\n'
} >"$summary_manifest"
...
echo "AIReady sustained-readiness history trend audit completed"
echo "audit_root: $output_root"
echo "requested_history_window: $count"
echo "history_audits_checked: $((validated_count + failed_count))"
echo "history_audits_passed: $validated_count"
